...
export function buildHand({ tableSize = 8, scheme = 'gg', unit = 'bb', sb = 0.5, bb = 1, ante = 0, anteMode = 'none', seats, hero, board = '', actions = [] }) {
  ...
  // A hand starts with only the blinds, so every seat named here is switched on
  // and anything else stays off.
  const wanted = Object.keys(seats);
  for (const position of seatRing(tableSize, scheme)) {
    hand = togglePlayer(hand, position, wanted.includes(position));
  }
